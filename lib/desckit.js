...
desckit.loadDesck = function (desckPath, callback) {
  ...
  function ready () {
    ...
    desck.conf.templateFile = desck.conf.templateFile ?
      path.normalize(desckPath + '/' + desck.conf.templateFile) :
      desckPath + '/script.ejs' ;
...
desckit.compileDesck = function (desck, callback) {
  ...
  function ready () {
    debug('now we\'re ready to render file, this is desck.locals:', desck.locals);
  ...
  if ('function' == typeof desck.render) {
    desck.render(function (err, result) {
      debug('Desck after callback script update, firing ready...');
      ready(); 
    });
  } else {
    ready();
